...
def show_current_descriptions_text(event, combo, frame):  # для нужного текста
    ...
    if current_method == "Decoloration":
        ...
        frame.text.insert(END,
                          """Two colors: Выберите нужные цвета придел замены и замастите всё изабражение только ими
                          """)
    elif current_method == "Pixelization":
        frame.text.insert(END,
                          """Standard: Выберити размер пиксилей и замастите ими всё изображение, compress mode позволяет сжать картинку обрезав лишнии пиксели
                          """)
